...
이 가이드는 `git.nnotion.kr` Gitea 서버에서 Personal Access Token(PAT)을 발급받고, Mac에서 설정하는 방법을 안내합니다.
...
# Keychain Access에서 git.nnotion.kr 항목 삭제
# 1. Cmd + Space → 키체인 접근
# 2. git.nnotion.kr 검색
# 3. 해당 항목 삭제
...
# Keychain에서 특정 호스트 인증정보 찾기
security find-internet-password -s git.nnotion.kr
```
